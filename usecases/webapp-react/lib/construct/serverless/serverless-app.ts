...
export class ServerlessApp extends Construct {
  ...
  constructor(
    ...
    const s3InterfaceEndpoint = props.s3InterfaceEndpoint;
    const s3NiIds = props.vpc.availabilityZones.map((az, index) => Fn.select(index, s3InterfaceEndpoint.vpcEndpointNetworkInterfaceIds))
    console.log(`DEBUG: ${JSON.stringify(s3NiIds)}`)
